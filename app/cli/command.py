...
import app.cli
from app.modules import initialize_sys_path, load_script
...
def load_internal_commands():
    """ Load ``commands`` from ``app.cli`` """
    for path in app.cli.__path__:
        for module in Path(path).iterdir():
            if module.suffix != '.py':
                continue
            __import__(f'app.cli.{module.stem}')
    ...
    for path in app.addons.__path__:
        for fullpath in Path(path).glob(f'*/cli/{command}.py'):
            if (found_command := fullpath.stem) and Command.is_valid_name(found_command):
                # loading as app.cli and not app.addons.{module}.cli
                # so it doesn't load app.addons.{module}.__init__
                mapping[f'app.cli.{found_command}'] = fullpath 
...
def find_command(name: str) -> Command | None:
    ...
    # import from app.cli
    with contextlib.suppress(ImportError):
        __import__(f'app.cli.{name}')
        return commands[name]

    # import from app.addons.*.cli
