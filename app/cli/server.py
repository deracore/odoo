...
from app.release import author as __author__  # noqa: F401
from app.release import version as __version__  # noqa: F401
from app.service import server
from app.tools import config

from . import Command

# Also use the `app` logger for the main script.
...
def report_configuration():
    ...
    import app.release  # noqa: PLC0415
    _logger.info("DERACore version %s", app.release.version)
    if os.path.isfile(config['config']):
        _logger.info("Using configuration file at %s", config['config'])
    _logger.info('addons paths: %s', app.addons.__path__)
    ...
    if sys.version_info[:2] > app.release.MAX_PY_VERSION:
        _logger.warning("Python %s is not officially supported, please use Python %s instead",
            '.'.join(map(str, sys.version_info[:2])),
            '.'.join(map(str, app.release.MAX_PY_VERSION))
        )
...
def setup_pid_file():
    ...
    This function assumes the configuration has been initialized.
    """
    import app  # for evented  # noqa: PLC0415
    if not app.evented and config['pidfile']:
        pid = os.getpid()
...
def main(args):
    ...
    for db_name in config['db_name']:
        from app.service import db  # noqa: PLC0415
        try:
...
class Server(Command):
    """Start the app server (default command)"""
