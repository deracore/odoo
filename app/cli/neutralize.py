...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = app.tools.config.parser
        ...
        opt = app.tools.config.parse_config(args, setup_logging=True)
        ...
        try:
            with app.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = app.modules.neutralize.get_installed_modules(cursor)
                    queries = app.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    app.modules.neutralize.neutralize_database(cursor)
