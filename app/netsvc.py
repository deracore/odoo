...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "app.netsvc.log is deprecated starting DERACore 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('app.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('app.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='app.tools.image')
...
DEFAULT_LOG_CONFIGURATION = [
    'app.http.rpc.request:INFO',
    'app.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['app:DEBUG', 'app.sql_db:INFO', 'app.http.rpc:DEBUG'],
    'debug_rpc': ['app:DEBUG', 'app.sql_db:INFO', 'app.http.rpc.request:DEBUG'],
    'debug': ['app:DEBUG', 'app.sql_db:INFO'],
    'debug_sql': ['app.sql_db:DEBUG'],
    'info': [],
    'runbot': ['app:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['app:WARNING', 'werkzeug:WARNING'],
    'error': ['app:ERROR', 'werkzeug:ERROR'],
    'critical': ['app:CRITICAL', 'werkzeug:CRITICAL'],
}
