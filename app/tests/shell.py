...
TEST_MODULE_NAME_PATTERN = re.compile(r'^app\.addons\.\w+\.tests')


def run_tests(env, test_tags, modules=None, reload_tests=False):
    """Run tests for the given modules and test tags."""

    if app.cli.COMMAND != 'shell':
        _logger.error('run_tests should be used only in app shell')
        return

    if app.tools.config['workers'] != 0:
        _logger.error('run_tests should be used only in threaded mode')
        return

    from app.service.server import server  # noqa: PLC0415
    if not server.httpd:
    ...
    app.tools.config['test_tags'] = test_tags
    app.tools.config['test_enable'] = True
    report = _run_tests(env.cr.dbname, modules)
    app.tools.config['test_enable'] = None
    app.tools.config['test_tags'] = None
