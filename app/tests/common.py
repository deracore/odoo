# -*- coding: utf-8 -*-
"""
The module :mod:`app.tests.common` provides unittest test cases and a few
...
import app.addons.base
import app.cli
import app.http
import app.models
import app.orm.registry
from app import api
from app.exceptions import AccessError
from app.fields import Command
from app.modules.registry import Registry, DummyRLock
from app.service import security
from app.sql_db import Cursor, Savepoint
from app.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from app.tools.mail import single_email_re
from app.tools.misc import find_in_path, lower_logging
from app.tools.xml_utils import _validate_xml
from app.addons.base.models import ir_actions_report

from . import case, test_cursor
from .result import appTestResult
...
if app.cli.COMMAND in ('server', 'start') and not config['test_enable']:
    _logger.error(
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: app.tests.common.Form is deprecated, use app.tests.Form",
...
# The app library is supposed already configured.
HOST = '127.0.0.1'
...
def get_db_name():
    dbnames = app.tools.config['db_name']
...
def release_test_lock():
    ...
    try:
        ...
        if not _registry_test_lock.acquire(timeout=60):
            tag = app.modules.module.current_test.canonical_tag
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding DERACore module name.
    """
    def register(func):
        # register func by app module name
        if func.__module__.startswith('app.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most app operations;
    """
    if not login:
...
class BaseCase(case.TestCase):
    """ Subclass of TestCase for DERACore-specific code. This class is abstract and
    ...
    def __init_subclass__(cls):
        ...
        if cls.__module__.startswith('app.addons.'):
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~app.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(app.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            app.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = app.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('app.sql_db.Cursor.execute', execute),
        ...
        for record in records:
            ...
            for field_name in field_names:
                ...
                match record._fields[field_name]:
                    case app.fields.Many2one():
                        record_value = record_value.id
                    case app.fields.One2many() | app.fields.Many2many():
                        record_value = sorted(record_value.ids)
                    case app.fields.Float() as field if digits := field.get_digits(record.env):
                        record_value = Approx(record_value, digits[1], decorate=False)
                    case app.fields.Monetary() as field if currency_field_name := field.get_currency_field(record):
    ...
    def assertCanOpenTestCursor(self):
        ...
        if app.modules.module.current_test != self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {app.modules.module.current_test.canonical_tag}"
    ...
    def get_method_additional_tags(self, test_method):
        ...
        """
        additional_tags = []
        if app.tools.config['test_tags'] and 'is_query_count' in app.tools.config['test_tags']:
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | app.addons.base.models.res_currency.ResCurrency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    """
    muted_registry_logger = mute_logger(app.orm.registry._logger.name)
    freeze_time = None
    ...
    def setUpClass(cls):
        ...
        cls._crypt_context_patcher = patch('app.addons.base.models.res_users.ResUsersPatchedInTest._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(app.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        if scs := app.tools.config['screencasts']:
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way DERACore widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    @classmethod
    def http_port(cls):
        if app.service.server.server is None:
            return None
        return app.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('app.service.http.request.')]
        ...
        if request_threads:
            ...
            app.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        app.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            app.http.root.session_store.delete(self.session)

        self.session = session = app.http.root.session_store.new()
        session.update(app.http.get_default_session(), db=get_db_name())
        session.context['lang'] = app.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('app.addons.base.models.res_users.ResUsersPatchedInTest._check_credentials', new=patched_check_credentials):
        ...
        app.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, success_signal=DEFAULT_SUCCESS_SIGNAL, debug=False, cpu_throttling=None, **kw):
        ...
        with self.allow_requests(browser=browser), contextlib.ExitStack() as atexit:
            ...
            if "bus.bus" in self.env.registry:
                from app.addons.bus.websocket import CloseCode, _kick_all, WebsocketConnectionHandler  # noqa: PLC0415
                from app.addons.bus.models.bus import BusBus  # noqa: PLC0415
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', f"app.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"app.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('app.http.Request._get_profiler_context_manager', route_profiler))
    ...
    def get_method_additional_tags(self, test_method):
        ...
        if app.tools.config['test_tags'] and 'is_tour' in app.tools.config['test_tags']:
...
def tagged(*tags):
    ...
    By default, all Test classes from app.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in DERACore test suites
