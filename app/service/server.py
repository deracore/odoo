...
from app.release import nt_service_name
from app.tools import config, gc, osutil, OrderedSet, profiler
from app.tools.cache import log_ormcache_stats
from app.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    import app  # for eventd
    if app.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(CommonRequestHandler):
    def setup(self):
        ...
        me.name = 'app.service.http.request.%s' % (me.ident,)
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        import app.addons  # noqa: PLC0415
        for path in app.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="app.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from app.addons.base.models.ir_cron import IrCron  # noqa: PLC0415
    ...
    def cron_spawn(self):
        ...
        for i in range(config['max_cron_threads']):
            t = threading.Thread(target=self.cron_thread, args=(i,), name=f"app.service.cron.cron{i}")
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from app.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('app: %s %s %s' % (self.__class__.__name__, self.pid, title))
...
def load_server_wide_modules():
    from app.modules.module import load_openerp_module  # noqa: PLC0415
    ...
    for dbname in dbnames:
        ...
        try:
            with preload_profiler:
                ...
                if config['test_enable']:
                    from app.tests import loader  # noqa: PLC0415
                    t0 = time.time()
...
def start(preload=None, stop=False):
    """ Start the app http server and cron processor.
    """
    global server

    load_server_wide_modules()
    import app.http  # noqa: PLC0415

    if app.evented:
        server = GeventServer(app.http.root)
    elif config['workers']:
        if config['test_enable']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(app.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which DERACore is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(app.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not app.evented:
        if inotify:
