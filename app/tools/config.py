...
from app.tools.func import classproperty
from . import appdirs
...
class configmanager:
    ...
    @property
    def rcfile(self):
        self._warn("Since 19.0, use app.tools.config['config'] instead", DeprecationWarning, stacklevel=2)
        return self['config']

    @rcfile.setter
    def rcfile(self, rcfile):
        self._warn(f"Since 19.0, use app.tools.config['config'] = {rcfile!r} instead", DeprecationWarning, stacklevel=2)
    ...
    def _build_cli(self):
        ...
        group.add_option("-c", "--config", dest="config", type='path', file_loadable=False, env_name='DERACORE_RC',
                         help="specify alternate config file")
        group.add_option("-s", "--save", action="store_true", dest="save", my_default=False, file_loadable=False,
                         help="save configuration to app.conf")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", type='path',  # sensitive default set in _load_default_options
                         help="Directory where to store DERACore data")
        ...
        group.add_option('--log-handler', action="append", type='comma', my_default=[':INFO'], metavar="MODULE:LEVEL",
                         help='setup a handler at LEVEL for a given MODULE. An empty MODULE indicates the root logger. '
                              'This option can be repeated. Example: "app.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const=("app.http:DEBUG",),
                         help='shortcut for --log-handler=app.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const=("app.sql_db:DEBUG",),
                         help='shortcut for --log-handler=app.sql_db:DEBUG')
        ...
        group.add_option("--db_app_name", dest="db_app_name", my_default="app-{pid}", env_name='PGAPPNAME',
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate DERACore to another language. "
    ...
    def _load_default_options(self):
        ...
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'app.conf')
        elif os.path.isfile(rcfilepath := os.path.expanduser('app.conf')):
            pass
        else:
            rcfilepath = 'app.conf'
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes app.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            app.tools.config.parse_config(sys.argv[1:])
        """
        from app import modules, netsvc  # noqa: PLC0415
        ...
        if setup_logging is not False:
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of DERACore 18, it's recommended to specify whether"
                    " you want DERACore to setup its own logging (or want to"
                    " handle it yourself)",
    ...
    def _load_cli_options(self, opt):
        # app.cli.command.main parses the config twice, the second time
