"""
This code is what let us use ES6-style modules in app.
Classic DERACore modules are composed of a top-level :samp:`app.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@app-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** app-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual app one with minimal changes.
...
from app.tools.misc import OrderedSet


def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom app modules.
...
def url_to_module_path(url):
    """
    DERACore modules each have a name. (app.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_odoo_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the app.define call.
    ...
    """
    return f"""app.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
ODOO_MODULE_RE = re.compile(r"""
    ...
    @app-module                       # '@app-module' statement
...
def is_odoo_module(url, content):
    """
    Detect if the file is a native app module.
    We look for a comment containing @app-module.

    :param url:
    :param content: source code
    :return: is this a app module that need transpilation ?
    """
...
def get_aliased_odoo_define_content(module_path, content):
    ...
    Example:
    If we have a require call somewhere in the app source base being:
    ...
    > app.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @app-module */
        // after
        /** @app-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @app-module */
        // after
        /** @app-module alias=web.AbstractAction default=false */
    ...
    if matchobj:
        ...
        if alias:
            if matchobj['default']:
                return """\napp.define(`%s`, ['%s'], function (require) {
                        return require('%s');
