...
from app.addons.account.tests.common import AccountTestMockOnlineSyncCommon
...
@app.tests.tagged('post_install', '-at_install', 'is_tour')
class TestUi(BaseTestUi):
    ...
    def test_company_switch_access_error_debug(self):
        ...
        with mute_logger("app.http"):
            self.start_tour(f"/app/action-{act_window.id}?debug=assets", "test_company_switch_access_error", login="admin", cookies={"cids": current_companies})
