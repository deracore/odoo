...
from app import http, modules
from app.exceptions import AccessError
from app.http import request
from app.tools import file_open, file_path
from app.tools.image import image_process
...
class WebManifest(http.Controller):
    ...
    def _get_webmanifest(self):
        web_app_name = request.env['ir.config_parameter'].sudo().get_param('web.web_app_name', 'DERAcore')
        manifest = {
            'name': web_app_name,
            'scope': '/app',
            'start_url': '/app',
        ...
        manifest['icons'] = [{
            'src': '/web/static/img/app-icon-%s.png' % size,
            'sizes': size,
            'type': 'image/png',
    ...
    def _icon_path(self):
        return 'web/static/img/app-icon-192x192.png'
    ...
    def scoped_app(self, app_id, path='', app_name=''):
        ...
        scoped_app_values = {
            'app_id': app_id,
            'apple_touch_icon': '/web/static/img/app-icon-ios.png',
            'app_name': app_name,
            'path': path,
    ...
    def scoped_app_manifest(self, app_id, path, app_name=''):
        """ Returns a WebManifest dedicated to the scope of the given app. A custom scope and start
            url are set to make sure no other installed PWA can overlap the scope (e.g. /app)
        """
        path = unquote(path)
        app_name = unquote(app_name) if app_name else self._get_scoped_app_name(app_id)
        webmanifest = {
