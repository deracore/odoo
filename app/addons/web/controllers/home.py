...
from app.tools.misc import hmac
...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if request.httprequest.method == 'POST':
            ...
            except app.exceptions.AccessDenied as e:
                if e.args == app.exceptions.AccessDenied().args:
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = app.SUPERUSER_ID
    ...
    def health(self, db_server_status=False):
        ...
        if db_server_status:
            try:
                app.sql_db.db_connect('postgres').cursor().close()
