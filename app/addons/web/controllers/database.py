...
from app.http import content_disposition, dispatch_rpc, request, Response
from app.service import db
from app.tools.misc import file_open, str2bool
from app.tools.translate import _

from app.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = app.tools.config.verify_admin_password('admin')
        d['list_db'] = app.tools.config['list_db']
        d['langs'] = app.service.db.exp_list_lang()
        d['countries'] = app.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = app.service.db.list_db_incompatible(d['databases'])
        except app.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = app.tools.config.verify_admin_password('admin')
        ...
        try:
            ...
            with app.modules.registry.Registry(name).cursor() as cr:
                env = app.api.Environment(cr, None, {})
    ...
    def backup(self, master_pwd, name, backup_format='zip', filestore=True):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            app.service.db.check_super(master_pwd)
            ...
            dump_stream = app.service.db.dump_db(name, None, backup_format, filestore)
