...
import { keyDown, queryOne, pointerDown } from "@app/hoot-dom";
...
const template = `
    <div class="mb-3 field-password pt-2 app_caps_lock_warning">
...
test("caps_lock_warning is started when there is a presence of password input field inside `.app_caps_lock_warning`", async () => {
...
test("caps lock alert is displayed when CapsLock is turned on", async () => {
    ...
    expect(".app_caps_lock_warning_text").toBeVisible();
});
