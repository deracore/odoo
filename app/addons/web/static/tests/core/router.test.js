...
import { click, on } from "@app/hoot-dom";
import { mockMatchMedia, tick } from "@app/hoot-mock";
...
describe("stateToUrl", () => {
    test("encodes URI compatible strings", (assert) => {
        expect(router.stateToUrl({})).toBe("/app");
        expect(router.stateToUrl({ a: "11", b: "summer wine" })).toBe("/app?a=11&b=summer%20wine");
    ...
    test("backwards compatibility: no action stack, action encoded in path", (assert) => {
        expect(router.stateToUrl({})).toBe("/app");
        // action
        expect(router.stateToUrl({ action: "some-path" })).toBe("/app/some-path");
        expect(router.stateToUrl({ active_id: 5, action: "some-path" })).toBe("/app/5/some-path");
        ...
        expect(router.stateToUrl({ action: "some-path", resId: 2 })).toBe("/app/some-path/2");
        ...
        expect(router.stateToUrl({ action: 1, resId: 2 })).toBe("/app/action-1/2", {
        ...
        // model
        expect(router.stateToUrl({ model: "some.model" })).toBe("/app/some.model");
        expect(router.stateToUrl({ model: "some.model", resId: 2 })).toBe("/app/some.model/2");
        expect(router.stateToUrl({ active_id: 5, model: "some.model" })).toBe("/app/5/some.model");
    ...
    test("actionStack: one action", () => {
        expect(router.stateToUrl({ actionStack: [] })).toBe("/app");
        // action
...
describe("Scoped apps", () => {
    test("url location is changed to /app if the client is not used in a standalone scoped app", async () => {
        Object.assign(browser.location, { pathname: "/scoped_app/some-path" });
        createRouter();
        router.pushState({ app_name: "some_app", path: "scoped_app/some_path" });
        await tick();
        expect(browser.location.href).toBe("https://www.hoot.test/app/some-path?app_name=some_app&path=scoped_app%2Fsome_path");
    });
