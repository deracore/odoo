...
import { press, queryAllTexts } from "@app/hoot-dom";
...
test("displays only apps if the search value is '/'", async () => {
    ...
    expect(".app_command_category").toHaveCount(1);
    expect(".app_command").toHaveCount(2);
    expect(queryAllTexts(".app_command_name")).toEqual(["Contact", "Sales"]);
});
...
test("displays apps and menu items if the search value is not only '/'", async () => {
    ...
    await press(["control", "k"]);
    await animationFrame();
    await contains(".app_command_palette_search input").edit("/sal", { confirm: false });
    await animationFrame();
    expect(".app_command_palette").toHaveCount(1);
    expect(".app_command").toHaveCount(3);
    expect(queryAllTexts(".app_command_name")).toEqual(["Sales", "Sales / Info", "Sales / Report"]);
});
...
test("opens an app", async () => {
    ...
    await animationFrame();
    expect(".app_menu_brand").toHaveText("Contact");
...
test("opens a menu items", async () => {
    ...
    await animationFrame();
    expect(".app_command_palette").toHaveCount(1);
    expect(".app_command_category").toHaveCount(2);

    await contains("#o_command_2").click();
    await animationFrame();
    // empty screen for now, wait for actual action to show up
    await animationFrame();
    expect(".app_menu_brand").toHaveText("Sales");
