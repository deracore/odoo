...
import { animationFrame, press } from "@app/hoot-dom";
import { Deferred } from "@app/hoot-mock";
import {
    contains,
    makeMockEnv,
    mountWithCleanup,
...
test("Barcode scanner crop overlay", async () => {
    ...
    const firstBarcodeValue = "DERAcore";
    ...
    await videoReady;
    await contains(".app_crop_icon").dragAndDrop(".app_crop_container", {
        relative: true,
        position: {
            x: 0,
            y: 0,
...
test("Closing barcode scanner before camera loads should not throw an error", async () => {
    ...
    expect(".app-barcode-modal").toHaveCount(1)

    await press("escape");

    await animationFrame();
    expect(".app-barcode-modal").toHaveCount(0)
