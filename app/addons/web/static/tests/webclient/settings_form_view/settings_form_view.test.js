...
import { animationFrame, Deferred, mockSendBeacon, mockTouch, runAllTimers } from "@app/hoot-mock";
import {
    clickModalButton,
    clickSave,
    defineActions,
...
test("change setting on nav bar click in base settings on desktop", async () => {
    await mountView({
        ...
        arch: /* xml */ `
            <form string="Settings" class="oe_form_configuration app_base_settings" js_class="base_settings">
    ...
    expect(queryAllTexts(".settings .app_settings_container .app_form_label")).toEqual([
        "Bar",
        "Big BAZ",
        "Foo",
        "Personalize setting",
    ...
    expect(".app_form_editable").not.toHaveClass("app_form_nosheet");
    expect(".app_searchview input").toBeFocused({ message: "searchview input should be focused" });
    expect(".app_settings_block:not(.d-none) .app_settings_header").toHaveCount(1);
    expect(".app_setting_box a").toHaveCount(2);
    expect(".app_setting_box span.fa:eq(0)").toHaveAttribute(
        "title",
        "this is bar info"
    );
    expect(".app_setting_box span.fa:eq(1)").toHaveAttribute(
        "title",
        "this is foo info"
    );
    expect(".app_setting_box a:eq(0)").toHaveAttribute(
        "href",
        "https://www.odoo.com/documentation/1.0/applications/technical/web/settings/this_is_a_test.html"
    );
    expect(".app_setting_box a:eq(1)").toHaveAttribute(
        "href",
    ...
    expect(queryAllTexts(".app_settings_container .app_setting_box .app_form_label")).toEqual(
        ["Bar", "Big BAZ"],
    ...
    expect(queryAllTexts(".app_settings_container .app_setting_box .app_form_label")).toEqual(["Bar"], {
        message: "Foo is not shown",
    });
    expect(".app_settings_block:not(.d-none) .app_settings_header").toHaveCount(1);
    ...
    expect(".app_nocontent_help").toBeVisible({ message: "record not found message shown" });
    expect(".app_settings_block:not(.d-none) .app_settings_header").toHaveCount(0);
...
test("don't show noContentHelper if no search is done", async () => {
    ...
    expect(".app_nocontent_help").not.toHaveCount();
});
    ...
    await mountView({
        ...
        arch: /* xml */ `
            <form string="Settings" class="oe_form_configuration app_base_settings" js_class="base_settings">
                <app string="CRM" name="crm">
                    <block title="Baz">
                        <field name="baz" class="app_light_label" widget="radio"/>
                    </block>
                </app>
            </form>
        `,
...
test("hide / show setting tips properly", async () => {
    ...
    expect(".app_setting_tip:not(.d-none)").toHaveCount(1, {
    ...
    await runAllTimers();
    await animationFrame();
    expect(".app_setting_tip:not(.d-none)").toHaveCount(0, {
...
test("settings views does not read existing id when coming back in breadcrumbs", async () => {
    ...
    expect(".app_field_boolean input").toHaveProperty("disabled", false);
    await click("button[name='4']");
    await animationFrame();
    expect(".breadcrumb").toHaveText("Settings");
    await click(".app_control_panel .breadcrumb-item a");
    await animationFrame();
    expect(".app_field_boolean input").toHaveProperty("disabled", false);
    expect.verifySteps([
...
test("resIds should contains only 1 id", async () => {
    ...
    await mountView({
        ...
        arch: /* xml */ `
            <form string="Settings" class="oe_form_configuration app_base_settings" js_class="base_settings">
                <div class="app_setting_container">
                    <div class="settings">
                        <app string="CRM" name="crm">
                            <block>
    ...
    await click(".app_field_char .btn.app_field_translate"); // Translate
    await animationFrame();
...
test("settings views does not read existing id when reload", async () => {
    ...
    await click(".modal button.btn.btn-primary.app_form_button_save");
    await animationFrame();
...
test("Auto save: don't save on closing tab/browser", async () => {
    ...
    expect(".app_field_boolean input:checked").toHaveCount(0, {
        message: "checkbox should not be checked",
    });
    expect(".app_dirty_warning").toHaveCount(0, { message: "warning message should not be shown" });
    await click(".app_field_boolean input[id=bar_0]");
    await animationFrame();
    expect(".app_field_boolean input:checked").toHaveCount(1, {
...
test("Auto save: don't save on visibility change", async () => {
    ...
    expect(".app_dirty_warning").toHaveCount(0, { message: "warning message should not be shown" });
    click(".app_field_boolean input[id=bar_0]");
    await animationFrame();
    expect(".app_field_boolean input:checked").toHaveCount(1, {
        message: "checkbox should be checked",
    });
...
test("settings views does not write the id on the url", async () => {
    ...
    await click(".app_field_boolean input");
    await animationFrame();
    expect(".app_field_boolean input").toBeChecked({ message: "checkbox should be checked" });
    await clickSave();
...
test("search for default label when label has empty string", async () => {
    ...
    await getService("action").doAction(1);
    expect(".app_form_label").toHaveCount(1);
    expect(".app_form_label").toHaveText("");
    expect(".app_settings_block:not(.d-none) .settingSearchHeader").toHaveCount(0);
    await editSearch("Fo");
    await runAllTimers();
    expect(".app_form_label").toHaveCount(0);
    expect(".app_settings_block:not(.d-none) .settingSearchHeader").toHaveCount(0);
});
...
test("clicking on any button in setting should show discard warning if setting form is dirty", async () => {
    ...
    expect(".app_list_view").toHaveCount(1, { message: "should be open list view" });
    await click(".app_control_panel .breadcrumb-item a, .app_back_button");
    await animationFrame();
    expect(".app_field_boolean input:checked").toHaveCount(0, {
        message: "checkbox should not be checked",
    });
    ...
    expect(".app_form_view").toHaveCount(1, { message: "should be remain on form view" });

    await clickSave();
    expect(".modal").toHaveCount(0, { message: "should not open a warning dialog" });
    expect(".app_field_boolean input").toHaveProperty("disabled", false); // Everything must stay in edit

    await click(".app_field_boolean input");
    await animationFrame();
    await click(".app_control_panel .app_form_button_cancel"); // Form Discard button
    await animationFrame();
    expect(".modal").toHaveCount(0, { message: "should not open a warning dialog" });
...
test("header field don't dirty settings", async () => {
    ...
    await getService("action").doAction(1);
    expect(".app_field_boolean input").not.toBeChecked({ message: "checkbox should not be checked" });
...
test("click on save button which throws an error", async () => {
    ...
    expect.verifySteps(["get_views", "onchange"]);
    expect(".app_form_button_save").toHaveCount(1);
    expect(".app_form_button_save").toHaveProperty("disabled", false);
...
test("clicking a button with dirty settings -- discard", async () => {
    ...
    expect(".app_field_boolean[name='bar'] input").toBeChecked();
    expect(queryAllTexts`.app_field_tags .app_tag`).toEqual(["xphone", "xpad"]);
    expect(".app_tag_color_1").toHaveCount(1);
    expect(".app_tag_color_3").toHaveCount(1);
    await click(".app_field_boolean[name='foo'] input[type='checkbox']");
    await animationFrame();
    await click(".myBtn");
    await animationFrame();
    await click(".modal .btn-secondary:eq(1)");
...
test("settings view does not display app_not_app settings", async () => {
    await mountView({
        type: "form",
        resModel: "res.config.settings",
        arch: /* xml */ `
    ...
    expect(queryAllTexts(".settings .app_form_label")).toEqual(["Bar"]);
});
...
test("settings view shows a message if there are changes", async () => {
    ...
    expect(".app_field_boolean input").not.toBeChecked({ message: "checkbox should not be checked" });
    expect(".app_control_panel .app_dirty_warning").toHaveCount(0, {
        message: "warning message should not be shown",
    });
    await click(".app_field_boolean input[id=bar_0]");
    await animationFrame();
    expect(".app_field_boolean input").toBeChecked({ message: "checkbox should be checked" });
    expect(".app_control_panel .app_dirty_warning").toHaveCount(1, {
...
test("settings view shows a message if there are changes even if the save failed", async () => {
    ...
    await click(".app_control_panel .app_form_button_save");
    await animationFrame();
    expect.verifyErrors(["RPC_ERROR"]);
    expect(".app_control_panel .app_dirty_warning").toHaveCount(1, {
        message: "warning message should be shown",
...
test("execute action from settings view with several actions in the breadcrumb", async () => {
    ...
    await getService("action").doAction(1);
    expect(".app_breadcrumb").toHaveText("First action");
...
test("settings can contain one2many fields", async () => {
    ...
    await animationFrame();
    await click(".modal-footer .btn.app_form_button_save");
    await animationFrame();

    expect("table.app_list_table tr.app_data_row").toHaveText("Added Task", {
...
test("Discard button clean the settings view", async () => {
    ...
    await click(".app_control_panel .app_form_button_cancel");
    await animationFrame();
    expect(".app_field_boolean input").not.toBeChecked({ message: "checkbox should not be checked" });
...
test("Settings Radio widget: show and search", async () => {
                    ...
                    <block>
                        <setting>
                            ...
                            <div class="content-group">
                                <div class="mt16">
                                    <field name="product_id" class="app_light_label" widget="radio"/>
                                </div>
                            </div>
                        </setting>
                    </block>
    ...
    expect(queryAllTexts(".app_radio_item:has(label)")).toEqual(["xphone", "xpad"]);
    await editSearch("xp");
    await runAllTimers();
    expect(".highlighter").toHaveCount(2, { message: "should have 2 options highlighted" });
...
test("Settings with createLabelFromField", async () => {
    ...
    expect(queryAllTexts(".app_settings_container .app_setting_box .app_form_label")).toEqual(["Zab"]);
});
...
test("field and artificial label inside a settings page", async () => {
    ...
    expect(".app_field_integer[name=count]").toHaveCount(1);
    expect("span.app_form_label").toHaveInnerHTML(
        `<span searchabletext="\n                            items\n                        ">\n                            items\n                        </span>`
    );
});
...
test("highlight Element with inner html/fields", async () => {
    ...
    expect(".app_setting_right_pane .text-muted").toHaveText(
...
test("settings form doesn't autofocus", async () => {
    ...
    expect.verifySteps([
        `focusin: <input type="text" class="app_searchview_input app_input flex-grow-1 w-auto border-0" accesskey="Q" placeholder="Search..." role="searchbox">`,
    ]);
});
...
test("settings form keeps scrolling by app", async () => {
    ...
    // constrain app_content to have height for its children to be scrollable
    queryFirst(".app_content").style.setProperty("height", "200px");
...
test("BinaryField is correctly rendered in Settings form view", async () => {
    ...
    expect('.app_field_widget[name="file"] .fa-download').toHaveCount(1, {
        message: "Download button should be display in settings form view",
    });
    expect('.app_field_widget[name="file"].app_field_binary .app_input').toHaveValue("coucou.txt", {
...
test("Open settings from url, with app anchor", async () => {
    ...
    await animationFrame();
    expect(".selected").toHaveAttribute("data-key", "crm", { message: "crm setting selected" });
    expect(queryAllTexts(".settings .app_settings_container .app_form_label")).toEqual(["Foo"]);
});
...
test("Open settings from url, with setting id anchor", async () => {
    ...
    expect(".selected").toHaveAttribute("data-key", "crm", { message: "crm setting selected" });
    expect(queryAllTexts(".settings .app_settings_container .app_form_label")).toEqual(["Foo"]);
    expect(".app_setting_highlight").toHaveCount(1);
    expect(queryAllTexts(".settings .app_setting_highlight .app_form_label")).toEqual(["Foo"]);
    await runAllTimers();
    expect(".app_setting_highlight").toHaveCount(0);
});
...
test("Don't cache settings data", async () => {
    ...
    await animationFrame();
    expect(queryAllTexts(".settings .app_settings_container .app_form_label")).toEqual(["Bar"]);
