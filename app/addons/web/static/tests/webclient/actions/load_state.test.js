...
import { queryAllAttributes, queryAllTexts, queryFirst, runAllTimers } from "@app/hoot-dom";
import { animationFrame } from "@app/hoot-mock";
import { Component, onMounted, xml } from "@app/owl";
import {
    contains,
    defineActions,
    defineMenus,
...
function _t() {
    app.translationContext = "web";
    const translatedTerm = basic_t(...arguments);
    app.translationContext = null;
...
describe(`new urls`, () => {
    ...
    test(`action loading, when action not found, load previous`, async () => {
        redirect("/app/action-1001/action-666");
        logHistoryInteractions();
    ...
    test("properly reload dynamic actions from sessionStorage (action with id)", async () => {
        ...
        expect(browser.location.href).toBe("http://example.com/app/action-100/1/action-200");
    ...
    test("menu jumping fix: multiple menus sharing same action", async () => {
        ...
        redirect("/app/action-9001?menu_id=100");
        logHistoryInteractions();
        ...
        expect.verifySteps([
            ...
            "set current_lang-en",
            "pushState http://example.com/app/action-9001",
    ...
    test(`reload without lang change`, async () => {
        ...
        expect(router.stateToUrl(router.current)).toBe("/app/action-100/1/action-200");
