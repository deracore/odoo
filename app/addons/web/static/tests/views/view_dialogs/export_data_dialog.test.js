...
const openExportDialog = async () => {
    if (getMockEnv().isSmall) {
        await pointerDown(".app_data_row:nth-child(1)");
        await runAllTimers();
    } else {
        await contains(".app_list_record_selector input[type='checkbox']").click();
    }
    await contains(".app_control_panel .app_cp_action_menus .dropdown-toggle").click();
...
test("Export dialog UI test", async () => {
    ...
    expect(`.app_dialog .app_export_tree_item`).toHaveCount(3, {
        message: "There should be only three items visible",
    });
    await contains(".modal .app_export_search_input").edit("ac");
    expect(`.modal .app_export_tree_item`).toHaveCount(1, {
        message: "Only matching item is visible",
    });
    await contains(".modal .app_export_tree_item .app_add_field").click();
    expect(`.modal .app_export_field`).toHaveCount(2, {
        message: "There should be two fields in export field list",
    });
    expect(".modal .app_export_field:nth-child(2)").toHaveText("Activities");
    expect(".app_export_search_input").toHaveValue("ac", {
        message: "Search input still contains the search string",
    });
    await contains(".modal .app_export_search_input").edit("");
    expect(".modal .app_export_tree_item:nth-child(2) .app_tree_column").toHaveClass("fw-bolder");
    await contains(".modal .app_export_field:first-child .app_remove_field").click();
    expect(`.modal .app_export_field`).toHaveCount(1);
});
...
test("Export dialog: interacting with export templates", async () => {
    ...
    expect(`.app_dialog`).toHaveCount(1);
    expect(".app_export_tree_item:nth-child(2) .app_add_field").toHaveClass("app_inactive", {
        message: "fields already selected cannot be added anymore",
    });

    // load a template which contains the activity_ids field
    await select("1", { target: ".app_exported_lists_select" });
    await animationFrame();
    expect(`.app_fields_list .app_export_field`).toHaveCount(1);
    expect(`.app_fields_list .app_export_field`).toHaveText("Activities");
    await contains(".app_export_tree_item:nth-child(2) .app_add_field").click();
    expect(`.app_exported_lists_select`).toHaveCount(1);
    expect(`.app_save_list_btn`).toHaveCount(0);
    expect(`.app_cancel_list_btn .fa-undo`).toHaveCount(1);
    expect(`.app_fields_list .app_export_field`).toHaveCount(2);
    await contains(".app_cancel_list_btn").click();
    expect(`.app_fields_list .app_export_field`).toHaveCount(1, {
        message: "the template has been reset and the added field is no longer in the list",
    });
    await contains(".app_export_tree_item:nth-child(2) .app_add_field").click();
    await select("new_template", { target: ".app_exported_lists_select" });
    await animationFrame();
    expect(`.app_exported_lists_select`).toHaveCount(0);
    expect(`input.app_save_list_name`).toHaveCount(1, {
        message: "an input is present to edit the current template name",
    });
    await contains(".app_save_list_btn").click();
    expect(".app_notification").toHaveText("Please enter save field list name");
    await contains(".app_cancel_list_btn").click();
    expect(`.app_exported_lists_select`).toHaveCount(1);
    await contains(".app_export_tree_item:nth-child(3) .app_add_field").click();
    expect(`.app_fields_list .app_export_field`).toHaveCount(3);
    await select("new_template", { target: ".app_exported_lists_select" });
    await animationFrame();
    await contains(".app_save_list_name").edit("Export template");
    await contains(".app_save_list_btn").click();
    expect(".app_exported_lists_select").toHaveValue("2", {
        message: "the new template is now selected",
    });
    expect(queryAllTexts(".app_right_field_panel .app_export_field")).toEqual([
        "Activities",
        "Foo",
        "Bar",
    ]);
    await select("", { target: ".app_exported_lists_select" });
    await animationFrame();
    expect(queryAllTexts(".app_right_field_panel .app_export_field")).toEqual(
        ["Activities", "Foo", "Bar"],
        {
    ...
    expect(".app_delete_exported_list").toHaveCount(0, {
        message: "trash icon is not visible when no template has been selected",
    });
    await select("2", { target: ".app_exported_lists_select" });
    await animationFrame();
    await contains(".app_delete_exported_list").click();
    expect(queryAll(".app_dialog .modal-body")[1]).toHaveText(
        "Do you really want to delete this export template?"
    );
    await contains(".app-overlay-item:nth-child(2) .btn-primary").click();
    expect(".app_exported_lists_select").toHaveValue("", {
        message: "the template list has been reset",
    });
    expect(queryAllTexts(".app_right_field_panel .app_export_field")).toEqual(["Foo"]);
});
...
test("Export dialog: interacting with export templates in debug", async () => {
    ...
    expect(".app_fields_list .app_export_field").toHaveText("Foo (foo)");
    await select("1", { target: ".app_exported_lists_select" });
    await animationFrame();
    expect(".app_fields_list .app_export_field").toHaveCount(1);
    expect(".app_fields_list .app_export_field").toHaveText("Activities (activity_ids)");
});
...
test("Export dialog: interacting with available fields", async () => {
    ...
    const firstField = ".app_left_field_panel .app_export_tree_item:first-child ";
    await contains(firstField).click();

    // show then hide content for the 'partner_ids' field. Then show it again
    await contains(firstField + ".app_export_tree_item").click();
    await contains(firstField + ".app_export_tree_item").click();
    await contains(firstField + ".app_export_tree_item").click();
    // we should only make one rpc to fetch fields
    expect.verifySteps(["fetch fields for 'partner_ids'"]);
    expect(
        ".app_export_tree_item[data-field_id='activity_ids/partner_ids/company_ids'] .app_expand_parent"
    ).toHaveCount(0, {
        message: "available fields are limited to 2 levels of subfields",
    });
    await dblclick(".app_export_tree_item[data-field_id='activity_ids/partner_ids/company_ids']");
    await animationFrame();
    expect(
        ".app_export_tree_item[data-field_id='activity_ids/partner_ids/company_ids'] .app_add_field"
    ).toHaveClass("app_inactive", {
        message: "field has been added by double clicking on it and cannot be added anymore",
    });
    await contains(firstField + ".app_add_field").click();
    expect(queryAllTexts(".app_right_field_panel .app_export_field")).toEqual([
        "Foo",
        "Company",
        "Activities",
    ]);
    await dblclick(".app_export_tree_item[data-field_id='activity_ids']");
    await animationFrame();
    expect(queryAllTexts(".app_right_field_panel .app_export_field")).toEqual(
    ...
    await contains(".app_export_field:first-child").dragAndDrop(
        queryFirst(".app_export_field:nth-child(2)")
    );
    await contains(".app_export_field:nth-child(3)").dragAndDrop(
        queryFirst(".app_export_field:first-child")
    );
    expect(queryAllTexts(".app_right_field_panel .app_export_field")).toEqual([
        "Activities",
        "Company",
    ...
    await contains(".modal .app_export_field:nth-child(2) .app_remove_field").click();
    expect(queryAllTexts(".app_right_field_panel .app_export_field")).toEqual(["Activities", "Foo"]);
    await contains(
        firstField +
            ".app_export_tree_item[data-field_id='activity_ids/partner_ids/name'] .app_add_field"
    ).click();
    expect(queryAllTexts(".app_right_field_panel .app_export_field")).toEqual([
        "Activities",
        "Foo",
...
test("Export dialog: compatible and export type options", async () => {
    ...
    await openExportDialog();
    expect("input[name='app_export_format_name']").toHaveCount(3);
    expect("[name=o_export_format_name][value=csv]").toBeChecked();
    expect(".app_export_format div:nth-of-type(3)").toHaveText("WOW");
    await check(".app_export_format div:nth-of-type(3) input");
    await animationFrame();
    expect(".app_export_tree_item").toHaveCount(3);
    await contains(".app_import_compat input").click();
    expect(".app_export_tree_item").toHaveCount(3);
    def.resolve();
    await animationFrame();
    await contains(".app_select_button").click();
...
test("toggling import compatibility after adding an expanded field", async () => {
    ...
    await contains("[data-field_id='activity_ids/partner_ids'] .app_add_field").click();
    await contains(".app_import_compat input").click();
    await contains("[data-field_id='activity_ids']").click();
    await contains(".app_select_button").click();
    // download file has been called with the correct url
...
test("Export dialog: export list with 'exportable: false'", async () => {
    ...
    expect(".app_export_field").toHaveCount(2);
    expect(".app_fields_list").toHaveText("Foo\nExportable");
});
...
test("Export dialog: sortable on desktop", async () => {
    ...
    await contains(".modal .app_export_tree_item .app_add_field").click();
    expect(".app_export_field_sortable").toHaveCount(2, {
...
test("Export dialog: non-sortable on mobile", async () => {
    ...
    await contains(".modal .app_export_tree_item .app_add_field").click();
    expect(".app_export_field_sortable").toHaveCount(0, {
...
test("Export list with modified context", async () => {
    ...
    await contains(".app-dropdown-item:contains(OwO)").click();
    await exportAllAction();
...
test("Export dialog with duplicated fields on desktop", async () => {
    ...
    expect(".app_list_table th:nth-child(2)").toHaveText("Foo");
    expect(".app_list_table th:nth-child(3)").toHaveText("duplicate of Foo");
    await openExportDialog();
    expect(".modal .app_export_field").toHaveCount(1);
    expect(".modal .app_export_field").toHaveText("Foo", {
...
test("Export dialog with duplicated fields on mobile", async () => {
    ...
    expect(".app_list_table th:nth-child(1)").toHaveText("Foo");
    expect(".app_list_table th:nth-child(2)").toHaveText("duplicate of Foo");
    await openExportDialog();
    expect(".modal .app_export_field").toHaveCount(1);
    expect(".modal .app_export_field").toHaveText("Foo", {
        message: "the only field to export corresponds to the field displayed in the list view",
...
test("Export dialog: search subfields", async () => {
    ...
    await contains(firstField + ".app_export_tree_item").click();
    await contains(firstField + ".app_export_tree_item").click();
    await contains(".app_export_search_input").edit("company");
    expect(".app_export_tree_item[data-field_id='activity_ids/partner_ids/company_ids']").toHaveCount(
        1,
        {
...
test("Export dialog: expand subfields after search", async () => {
    ...
    await contains(firstField).click();
    await contains(firstField).click();
    await contains(".app_export_search_input").edit("Attendants");
    expect(".app_export_tree_item[data-field_id='activity_ids/partner_ids']").toHaveCount(1, {
        message: "subfield that was known has been found and is displayed",
    });
    await contains(".app_export_tree_item[data-field_id='activity_ids/partner_ids']").click();
...
test("Export dialog: search in debug", async () => {
    ...
    await openExportDialog();
    await contains(".app_left_field_panel .app_export_tree_item:first-child").click();
    await contains(".app_export_tree_item:first-child .app_export_tree_item").click();
    await contains(".app_export_search_input").edit("company_ids");
    expect(".app_export_tree_item[data-field_id='activity_ids/partner_ids/company_ids']").toHaveCount(
        1,
        {
...
test("Export dialog: disable button during export", async () => {
    ...
    await openExportDialog();
    expect(".app_select_button").toBeEnabled();
    await contains(".app_select_button").click();
    expect(".app_select_button").not.toBeEnabled();
    def.resolve();
    await animationFrame();
    expect(".app_select_button").toBeEnabled();
});
...
test("Export dialog: fields displayed in same Order as list view when export", async () => {
    ...
    await openExportDialog();
    expect(".modal .app_export_field:first-child").toHaveText("Demo", {
        message: "Field demo should appear first in the export list",
    });
    expect(".modal .app_export_field:nth-child(2)").toHaveText("Abc", {
