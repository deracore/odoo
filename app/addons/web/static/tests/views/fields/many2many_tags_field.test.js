...
import { hover, press, queryAllTexts, queryOne } from "@app/hoot-dom";
import { animationFrame, Deferred, runAllTimers } from "@app/hoot-mock";
...
test("Many2ManyTagsField with and without color on desktop", async () => {
    ...
    expect("[name=partner_ids] .app_tag").toHaveCount(0);
    await contains(".app-autocomplete--dropdown-item:eq(0)").click();
    expect("[name=partner_ids] .app_tag").toHaveCount(1);

    // Show the color list
    expect(".app_colorlist").toHaveCount(0);
    await contains("[name=partner_ids] .app_tag").click();
    expect(".app_colorlist").toHaveCount(1);
    await contains(getFixture()).click();

    // Add a tag to second field
    expect("[name=timmy] .app_tag").toHaveCount(0);
    await clickFieldDropdown("timmy");
    expect("[name='timmy'] .app-autocomplete.dropdown li").toHaveCount(3, {
        message: "autocomplete dropdown should have 3 entries (2 values + 'Search more...')",
    });
    await clickFieldDropdownItem("timmy", "gold");
    expect("[name=timmy] .app_tag").toHaveCount(1);
    expect(queryAllTexts(`.app_field_many2many_tags[name="timmy"] .badge`)).toEqual(["gold"]);

    // Show the color list
    expect(".app_colorlist").toHaveCount(0);
    await contains("[name=timmy] .app_tag").click();
    expect(".app_colorlist").toHaveCount(0);
});
...
test("Many2ManyTagsField with and without color on mobile", async () => {
    ...
    // Add a tag to first field
    expect("[name=partner_ids] .app_tag").toHaveCount(0);
    await contains("article.app_kanban_record:eq(0)").click();
    expect("[name=partner_ids] .app_tag").toHaveCount(1);
    ...
    expect("[name=timmy] .app_tag").toHaveCount(0);
    await clickFieldDropdown("timmy");
    expect("article.app_kanban_record").toHaveCount(2, {
...
test("Many2ManyTagsField with color: rendering and edition on desktop", async () => {
    ...
    expect(".app_field_many2many_tags .badge").toHaveCount(2);
    expect(".badge .app_tag_badge_text:eq(0)").toHaveText("gold");
    expect(".badge .app_tag_badge_text:eq(1)").toHaveText("silver");
    expect(".badge:eq(0)").toHaveClass("app_tag_color_2");
    expect(".app_field_many2many_tags .app_delete").toHaveCount(2);

    // add an other existing tag
    await contains("div[name='timmy'] .app-autocomplete.dropdown input").click();
    expect(`.dropdown-item .fw-bold`).toHaveCount(2);
    expect(queryAllTexts`.dropdown-item .fw-bold`).toEqual(["gold", "silver"]);
    expect(".app-autocomplete--dropdown-menu li").toHaveCount(4);
    expect(".app-autocomplete--dropdown-menu li a:eq(2)").toHaveText("red");

    await contains(".app-autocomplete--dropdown-menu li a:eq(2)").click();
    expect(".app_field_many2many_tags .badge").toHaveCount(3);
    expect(".app_field_many2many_tags .badge .app_tag_badge_text:eq(2)").toHaveText("red");
    expect(".badge:eq(2)").toHaveClass("app_tag_color_8");

    // remove tag silver
    await contains(".app_field_many2many_tags .app_delete:eq(1)").click();
    expect(".app_field_many2many_tags .badge").toHaveCount(2);
    const textContent = queryAllTexts(".app_field_many2many_tags  .dropdown-toggle .badge");
    ...
    const badgeElement = queryOne(".app_field_many2many_tags .badge:eq(1)"); // selects 'red' tag
    await contains(badgeElement).click();
    expect(".app_tag_popover .form-check input").toHaveCount(1);

    expect(".app_tag_popover .form-check input").not.toBeChecked();

    await contains(".app_tag_popover input[type='checkbox']").click();
    expect(badgeElement).toHaveAttribute("data-color", "0");

    await contains(badgeElement).click();
    expect(".app_tag_popover .form-check input").toBeChecked();
...
test("Many2ManyTagsField in list view on desktop", async () => {
    ...
    await contains(".app_field_many2many_tags .badge :nth-child(1)").click();
    expect.verifySteps(["selectRecord"]);
    await animationFrame();

    expect(".app_colorlist").toHaveCount(0);

    await contains(".app_list_record_selector:eq(1)").click();
...
test("Many2ManyTagsField view a domain on desktop", async () => {
    ...
    expect(".app_field_many2many_tags .badge").toHaveCount(1);
    expect(queryAllTexts(".badge")).toEqual(["gold"]);

    await clickFieldDropdown("timmy");

    expect(".app-autocomplete--dropdown-menu li").toHaveCount(3);

    expect(".app-autocomplete--dropdown-menu li a:eq(0)").toHaveText("gold");
...
test("Many2ManyTagsField view a domain on mobile", async () => {
    ...
    expect(".app_kanban_record span").toHaveCount(2);

    expect(".app_kanban_record span:eq(0)").toHaveText("gold");
...
test("use binary field as the domain on desktop", async () => {
    ...
    expect(".app-autocomplete--dropdown-menu li").toHaveCount(3);
    expect(queryAllTexts(".app-autocomplete--dropdown-menu li")).toEqual([
        "gold",
        "silver",
        "Search more...",
    ]);
...
test("use binary field as the domain on mobile", async () => {
    ...
    expect(".app_kanban_record span").toHaveCount(2);
    expect(queryAllTexts(".app_kanban_record span")).toEqual(["gold", "silver"]);
    expect(".app_kanban_record span:eq(0)").toHaveText("gold");
...
test("Domain: allow python code domain in fieldInfo on desktop", async () => {
    ...
    await clickFieldDropdown("timmy");
    expect(".app-autocomplete--dropdown-menu li").toHaveCount(2);
    expect(".app-autocomplete--dropdown-menu li a:eq(0)").toHaveText("silver");
...
test("Many2ManyTagsField in a new record on desktop", async () => {
    ...
    expect(".app_field_many2many_tags .badge").toHaveCount(1);
    expect(queryAllTexts(".app_field_many2many_tags .badge")).toEqual(["gold"]);
...
test("Many2ManyTagsField: update color", async () => {
    ...
    const badgeNode = queryOne(".app_tag.badge");
    ...
    await contains('.app_colorlist button[data-color="1"]').click();
    expect(badgeNode).toHaveAttribute("data-color", "1");

    // Update the color in edit => write on save with rest of the record
    await contains(badgeNode).click();
    await contains('.app_colorlist button[data-color="6"]').click();
    await animationFrame();
    ...
    /*
    badgeNode = queryOne(".app_tag.badge"); // need to refresh the reference

    // Update the color in edit without save => we don't go through RPC
    // so it's not saved and it is lost on discard.
    await clickEdit();
    await contains(badgeNode).click();
    await contains('.app_colorlist button[data-color="8"]').click();
    await animationFrame();
...
test("Many2ManyTagsField with no_edit_color option", async () => {
    ...
    await contains(".app_tag.badge").click();
    expect(".app_colorlist").toHaveCount(0);
});
...
test("Many2ManyTagsField in editable list", async () => {
    ...
    expect(".app_data_row:nth-child(1) .app_field_many2many_tags .badge").toHaveCount(1);

    // edit first row
    await contains(".app_data_row:nth-child(1) .app_many2many_tags_cell").click();

    expect(
        ".app_data_row:nth-child(1) .app_many2many_tags_cell .app_field_many2many_selection"
    ).toHaveCount(1);

    // add a tag
    await selectFieldDropdownItem("timmy", "silver");

    expect(".app_data_row:nth-child(1) .app_field_many2many_tags .badge").toHaveCount(2);
});
...
test("Many2ManyTagsField can load more than 40 records", async () => {
    ...
    expect('.app_field_widget[name="partner_ids"] .badge').toHaveCount(100);
});
...
test("Many2ManyTagsField keeps focus when being edited", async () => {
    ...
    expect(".app_field_many2many_tags .badge").toHaveCount(0);
    expect("[name=foo] input").toBeFocused();

    await selectFieldDropdownItem("timmy", "gold");
    expect(".app_field_many2many_tags .badge").toHaveCount(1);
    expect(".app_field_many2many_tags input").toBeFocused();
});
...
test("Many2ManyTagsField: tags data-tooltip attribute", async () => {
    ...
    expect(".app_field_many2many_tags .app_tag.badge").toHaveAttribute("data-tooltip", "second record");
});
...
test("Many2ManyTagsField: toggle colorpicker with multiple tags", async () => {
    ...
    expect(".app_colorpicker").toHaveCount(0);

    // click on the badge to open colorpicker
    await contains(".app_field_many2many_tags .badge").click();
    expect(".app_colorlist").toHaveCount(1);

    await contains(".app_field_many2many_tags [data-tooltip=silver]").click();
    expect(".app_colorlist").toHaveCount(1);
...
test("Many2ManyTagsField: toggle colorpicker multiple times", async () => {
    ...
    expect(".app_field_many2many_tags .badge").toHaveCount(1);
    expect(".app_field_many2many_tags .badge").toHaveAttribute("data-color", "0");
    expect(".app_colorpicker").toHaveCount(0);
    ...
    await contains('.app_colorlist button[data-color="2"]').click();

    expect(".app_field_many2many_tags .badge").toHaveAttribute("data-color", "2");
    expect(".app_colorlist").toHaveCount(0);
});
...
test("Many2ManyTagsField: quick create a new record on desktop", async () => {
    ...
    await contains(".app_field_many2many_selection .app_input_dropdown input").edit("new", {
        confirm: false,
    });
    await runAllTimers();
    ...
    await clickSave();
    expect(".app_field_many2many_tags").toHaveText("new");
});
...
test("select a many2many value by pressing tab on desktop", async () => {
    ...
    expect(".app_field_many2many_tags .badge").toHaveCount(0);
    await contains(".app_field_many2many_tags input").edit("go", { confirm: false });
    await runAllTimers();
    await press("Tab");
    await animationFrame();
    expect(".app_field_many2many_tags .badge").toHaveCount(1);
    expect(".app_field_many2many_tags .badge").toHaveText("gold");

    await contains(".app_field_many2many_tags input").edit("r", { confirm: false });
    await runAllTimers();
    await press("ArrowDown");
    await press("Tab");
    await animationFrame();
    expect(".app_field_many2many_tags .badge").toHaveCount(2);
    expect(".app_field_many2many_tags .badge:eq(1)").toHaveText("red");
});
...
test("input and remove text without selecting any tag or option on desktop", async () => {
    ...
    // ensure no selection
    await hover(".app-autocomplete--dropdown-item:eq(0)");
    await hover(".app_form_renderer");
    await press("Tab");
    ...
    await contains(".app_field_many2many_tags input").clear({ confirm: false });
    await runAllTimers();
...
test("Many2ManyTagsField in one2many with name on desktop", async () => {
    ...
    expect(queryAllTexts(".app_data_cell")).toEqual(["second record\naaa"]);

    // open the x2m form view
    await contains('.app_field_one2many[name="turtles"] .app_data_cell').click();
    expect(queryAllTexts(".modal .app_data_cell")).toEqual(["blip", "My little Foo Value"]);
...
test("Many2ManyTagsField: select multiple records on desktop", async () => {
    ...
    expect(".app_dialog .app_list_renderer .app_list_record_selector input").toHaveCount(
        MockServer.env["partner.type"].length + 1
    );
    //multiple select tag
    await contains(".app_dialog .app_list_renderer .app_list_record_selector input").click();
    await animationFrame(); // necessary for the button to be switched to enabled.
    expect(".app_dialog .app_select_button").toBeEnabled();
...
test("Many2ManyTagsField: save&new in edit mode doesn't close edit window on desktop", async () => {
    ...
    expect('.app_field_many2many_tags[name="timmy"] .badge').toHaveCount(2);
});
...
test("Many2ManyTagsField: make tag name input field blank on Save&New on desktop", async () => {
    ...
    await contains(".app_field_widget input").edit("hello", { confirm: false });
    await runAllTimers();
    await clickFieldDropdownItem("timmy", "Create and edit...");

    expect(".modal .app_form_view input").toHaveValue("hello");

    // Create record with save & new
    await contains(".modal .btn-primary:nth-child(2)").click();
    expect(".modal .app_form_view input").toHaveValue("");
...
test("Many2ManyTagsField: Save&New in many2many_tags with default_ keys in context", async () => {
    ...
    expect(".modal .app_field_widget[name=name] input").toHaveValue("hello");
    expect(".modal .app_field_widget[name=color] input").toHaveValue("3");

    // Create record with save & new
    await contains(".modal .btn-primary:nth-child(2)").click();
    expect(".modal .app_field_widget[name=name] input").toHaveValue("");
    expect(".modal .app_field_widget[name=color] input").toHaveValue("3");
...
test("Many2ManyTagsField: conditional create/delete actions on desktop", async () => {
    ...
    expect(".app_field_many2many_tags.app_field_widget .badge .app_delete").toHaveCount(1);
    ...
    await contains(".app_field_many2many_tags input").edit("Something that does not exist", {
        confirm: false,
    });
    await runAllTimers();

    expect(queryAllTexts(`.app-autocomplete.dropdown li.app_m2o_dropdown_option`)).toEqual([
        'Create "Something that does not exist"',
        "Create and edit...",
    ]);

    // set turtle_bar false -> create and delete actions are no longer available
    await contains('.app_field_widget[name="turtle_bar"] input:eq(0)').click();
    await animationFrame();
    ...
    expect(".app-autocomplete.dropdown li.app_m2o_dropdown_option").toHaveCount(1);
    expect(
        ".app-autocomplete.dropdown li.app_m2o_dropdown_option a:contains(Search more...)"
    ).toHaveCount(1);
    ...
    await contains(".app_field_many2many_tags input").edit("Pa", { confirm: false });
    await runAllTimers();

    // only Search more option should be available
    expect(".app-autocomplete.dropdown li.app_m2o_dropdown_option").toHaveCount(1);
    expect(".app-autocomplete.dropdown li.app_m2o_dropdown_option a:contains(Search more)").toHaveCount(
        1
    );
});
...
test("failing many2one quick create in a Many2ManyTagsField on desktop", async () => {
    ...
    await contains(".app_field_many2many_tags input").edit("new partner", { confirm: false });
    await runAllTimers();
    ...
    expect(".modal .app_form_view").toHaveCount(1);
    expect(".modal .app_field_widget[name=name] input").toHaveValue("new partner");

    await contains(".modal .app_field_widget[name=color] input").edit(8);
...
test("navigation in tags (mode 'readonly') on desktop", async () => {
    ...
    expect("tr.app_data_row input[type=checkbox]").toBeFocused();

    await press("ArrowRight");

    expect("tr.app_data_row td[name=timmy]").toBeFocused();
});
...
test("navigation in tags (mode 'edit') on desktop", async () => {
    ...
    await contains("tr.app_data_row:eq(0) .app_many2many_tags_cell").click();

    expect("tr.app_data_row:eq(0) .app_field_many2many_tags .badge").toHaveCount(2);

    expect("tr.app_data_row:eq(0) [name=timmy] .app-autocomplete--input").toBeFocused();

    // press left to focus the rightmost facet
    await press("ArrowLeft");

    expect("tr.app_data_row:eq(0) [name=timmy] .badge:nth-child(2)").toBeFocused();

    // press left to focus the leftmost facet
    await press("ArrowLeft");

    expect("tr.app_data_row:eq(0) [name=timmy] .badge:nth-child(1)").toBeFocused();
    ...
    expect("tr.app_data_row:eq(0) [name=timmy] .badge:nth-child(1)").toBeFocused();
    expect("tr.app_data_row:eq(0) .app_field_many2many_tags .badge").toHaveCount(2);
    expect(queryAllTexts(".app_field_many2many_tags .badge")).toEqual(["gold", "silver"]);

    await press("BackSpace");
    await animationFrame();

    expect("tr.app_data_row:eq(0) .app_field_many2many_tags .badge").toHaveCount(1);
    expect(queryAllTexts(".app_field_many2many_tags .badge")).toEqual(["silver"]);
    expect("tr.app_data_row:eq(0) .app-autocomplete--input").toHaveCount(1);
});
...
test("Many2ManyTagsField with placeholder", async () => {
    ...
    expect(".app_field_widget[name='timmy'] input").toHaveAttribute("placeholder", "Placeholder");

    await selectFieldDropdownItem("timmy", "gold");

    expect(".app_field_widget[name='timmy'] input").toHaveAttribute("placeholder", "");
});
...
test("Many2ManyTagsField supports 'create' props to be a Boolean on desktop", async () => {
    ...
    await contains(".app_field_many2many_tags input").click();
    expect(".app_field_many2many_tags .app-autocomplete--dropdown-menu").toHaveText(
        "gold\nsilver\nSearch more..."
    );
});
...
test("Many2ManyTagsField supports 'create' props to be a Boolean on mobile", async () => {
    ...
    await contains(".app_field_many2many_tags input").click();
    expect(".app_kanban_renderer").toHaveText("gold\nsilver");
    expect(".modal-footer .btn").toHaveCount(2);
    expect(".modal-footer .btn.app_select_button").toHaveCount(1);
    expect(".modal-footer .btn.app_form_button_cancel").toHaveCount(1);
});
...
test("save a record with an empty many2many_tags required", async () => {
    ...
    await clickSave();
    expect("[name='timmy'].app_field_invalid").toHaveCount(1);
});
...
test("set a required many2many_tags and save directly", async () => {
    ...
    expect(".app_tag").toHaveText("", {
        message: "The tag is displayed, but the web read is not finished yet",
    });

    await clickSave();
    expect("[name='timmy']").not.toHaveClass("app_field_invalid");

    def.resolve();
    await animationFrame();
    expect(".app_tag").toHaveText("gold");
});
...
test("Many2ManyTagsField with option 'no_quick_create' set to true on desktop", async () => {
    ...
    expect(".app_tag").toHaveCount(0);
    await contains(".app_field_many2many_tags .app-autocomplete--input").edit("new tag", {
        confirm: false,
    });
    await runAllTimers();
    expect(".app-autocomplete.dropdown li.app_m2o_dropdown_option").toHaveCount(1);
    expect(".app-autocomplete.dropdown li.app_m2o_dropdown_option").toHaveClass(
        "app_m2o_dropdown_option_create_edit"
    );
    await clickFieldDropdownItem("timmy", "Create and edit...");
    expect(".modal").toHaveCount(1);
    expect(".modal .app_field_widget[name=name] input").toHaveValue("new tag");
    await contains(".modal .app_form_button_save").click();
    expect(".app_tag").toHaveCount(1);
    expect(".app_tag").toHaveText("new tag");
});
...
test("Many2ManyTagsField keep the linked records after discard of the quick create dialog on desktop", async () => {
    ...
    await contains(".modal .app_form_button_save").click();
    expect(".app_tag").toHaveCount(1);
    await contains(".app_field_many2many_tags .app-autocomplete--input").edit("tago", {
        confirm: false,
    });
    await runAllTimers();
    await clickFieldDropdownItem("timmy", "Create and edit...");
...
test("Many2ManyTagsField with option 'no_create' set to true on desktop", async () => {
    ...
    await runAllTimers();
    expect(".app-autocomplete.dropdown li.app_m2o_no_result").toHaveCount(1);
});
...
test("Many2ManyTagsField with attribute 'can_create' set to false on desktop", async () => {
    ...
    await runAllTimers();
    expect(".app-autocomplete.dropdown li.app_m2o_dropdown_option").toHaveCount(0);
    expect(".app-autocomplete.dropdown li.app_m2o_no_result").toHaveCount(1);
});
...
test("Many2ManyTagsField with arch context in form view on desktop", async () => {
    ...
    expect(".app_field_tags").toHaveText("gold coucou");
});
...
test("Many2ManyTagsField with arch context in list view on desktop", async () => {
    ...
    expect.verifySteps(["name search with context given", "read with context given"]);
    expect(".app_field_tags:eq(0)").toHaveText("gold coucou");
});
...
test("Many2ManyTagsField doesn't use virtualId for 'web_name_search' on desktop", async () => {
    ...
    await contains(".modal .app_form_button_save").click();
    expect(".modal").toHaveCount(0);
    expect("[name='turtles'] .app_tag_badge_text").toHaveCount(2);
    expect("[name='turtles'] .app_data_row").toHaveCount(2);
...
test("Many2ManyTagsField selected records still pickable and not duplicable on desktop", async () => {
    ...
    expect(".app_tag").toHaveCount(1);
    expect(".app_tag:eq(0)").toHaveText("leonardo");
    ...
    await contains("a.app_delete").click();
    expect(".app_tag").toHaveCount(0);
});
...
test("Many2ManyTagsField selected records still pickable and not duplicable on mobile", async () => {
    ...
    await contains("div[name='turtles']").click();
    await contains("input[id=turtles_0]").click();
    expect(`${".app_kanban_record"}:eq(0)`).toHaveText("leonardo");
...
test("Many2ManyTagsField with edit_tags option overrides color edition", async () => {
    ...
    expect(".app_field_widget[name=timmy] .app_badge").toHaveCount(2);
    expect(queryAllTexts(".app_field_widget[name=timmy] .app_badge")).toEqual(["gold", "silver"]);
    ...
    expect(".app_field_widget[name=timmy] .app_badge").toHaveCount(2);
    expect(queryAllTexts(".app_field_widget[name=timmy] .app_badge")).toEqual(["new", "silver"]);
...
test("search typeahead", async () => {
    ...
    await contains(".app_field_widget[name=timmy] input").click();
    await runAllTimers();
    expect.verifySteps([]);
    ...
    await contains(".app_field_widget[name=timmy] input").edit("g", { confirm: false });
    await runAllTimers();
    expect.verifySteps([]);
    expect(queryAllTexts(`.app-autocomplete.dropdown li`)).toEqual([
        "Start typing 3 characters",
    ...
    await contains(".app_field_widget[name=timmy] input").edit("go", { confirm: false });
    await runAllTimers();
    expect.verifySteps([]);
    expect(queryAllTexts(`.app-autocomplete.dropdown li`)).toEqual([
        "Start typing 3 characters",
    ...
    await contains(".app_field_widget[name=timmy] input").edit("gol", { confirm: false });
    await runAllTimers();
    expect.verifySteps(["web_name_search"]);
    expect(queryAllTexts(`.app-autocomplete.dropdown li`)).toEqual([
        "gold",
...
test("Many2ManyTagsField: press backspace multiple times to remove tag", async () => {
    ...
    await contains(".app_field_many2many_tags .badge:eq(1)").click();
    press("BackSpace");
    press("BackSpace");
    def.resolve();
    await animationFrame();
