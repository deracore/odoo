...
import { click, hover, leave, press, queryAll, queryAllTexts } from "@app/hoot-dom";
...
test("PriorityField when not set", async () => {
    ...
    expect(".app_field_widget .app_priority:not(.app_field_empty)").toHaveCount(1, {
        message: "widget should be considered set, even though there is no value for this field",
    });
    expect(".app_field_widget .app_priority a.app_priority_star").toHaveCount(2, {
        message:
            "should have two stars for representing each possible value: no star, one star and two stars",
    });
    expect(".app_field_widget .app_priority a.app_priority_star.fa-star").toHaveCount(0, {
        message: "should have no full star since there is no value",
    });

    expect(".app_field_widget .app_priority a.app_priority_star.fa-star-o").toHaveCount(2, {
...
test("PriorityField tooltip", async () => {
    ...
    const stars = queryAll(".app_field_widget .app_priority a.app_priority_star");
...
test("PriorityField in form view", async () => {
    ...
    expect(".app_field_widget .app_priority:not(.app_field_empty)").toHaveCount(1);
    expect(".app_field_widget .app_priority a.app_priority_star").toHaveCount(2);
    expect(".app_field_widget .app_priority a.app_priority_star.fa-star").toHaveCount(1);
    expect(".app_field_widget .app_priority a.app_priority_star.fa-star-o").toHaveCount(1);

    // click on the second star in edit mode
    await click(".app_field_widget .app_priority a.app_priority_star.fa-star-o:last");
    await animationFrame();

    expect(".app_field_widget .app_priority a.app_priority_star").toHaveCount(2);
    expect(".app_field_widget .app_priority a.app_priority_star.fa-star").toHaveCount(2);
    expect(".app_field_widget .app_priority a.app_priority_star.fa-star-o").toHaveCount(0);
});
...
test("PriorityField hover a star in form view", async () => {
    ...
    // hover last star
    const star = ".app_field_widget .app_priority a.app_priority_star.fa-star-o:last";
    await hover(star);
    await animationFrame();
    expect(".app_field_widget .app_priority a.app_priority_star").toHaveCount(2);
    expect(".app_field_widget .app_priority a.app_priority_star.fa-star").toHaveCount(2, {
...
test("PriorityField can write after adding a record -- kanban", async () => {
    ...
    expect(".app_kanban_record .fa-star").toHaveCount(0);

    await click(".app_priority a.app_priority_star.fa-star-o");
    // wait for web_save
    await animationFrame();
    expect.verifySteps(['web_save [[1],{"selection":"1"}]']);
    expect(".app_kanban_record .fa-star").toHaveCount(1);
    await click(".app_control_panel_main_buttons .app-kanban-button-new");
    await animationFrame();
    await animationFrame();
    await click(".app_kanban_quick_create .app_kanban_add");
    await animationFrame();
    expect.verifySteps(["web_save [[],{}]"]);
    await click(".app_priority a.app_priority_star.fa-star-o");
    await animationFrame();
    expect.verifySteps([`web_save [[6],{"selection":"1"}]`]);
    expect(".app_kanban_record .fa-star").toHaveCount(2);
});
...
test("PriorityField in editable list view", async () => {
    ...
    expect(".app_data_row:first-child .app_priority:not(.app_field_empty)").toHaveCount(1);
    expect(".app_data_row:first-child .app_priority a.app_priority_star").toHaveCount(2, {
        message:
            "should have two stars for representing each possible value: no star, one star and two stars",
    });
    expect(".app_data_row:first-child .app_priority a.app_priority_star.fa-star").toHaveCount(1, {
        message: "should have one full star since the value is the second value",
    });
    expect(".app_data_row:first-child .app_priority a.app_priority_star.fa-star-o").toHaveCount(1, {
    ...
    await click(".app_priority a.app_priority_star.fa-star");
    await animationFrame();

    expect(".app_data_row:first-child .app_priority a.app_priority_star").toHaveCount(2, {
        message: "should still have two stars",
    });
    expect(".app_data_row:first-child .app_priority a.app_priority_star.fa-star").toHaveCount(0, {
        message: "should now have no full star since the value is the first value",
    });
    expect(".app_data_row:first-child .app_priority a.app_priority_star.fa-star-o").toHaveCount(2, {
    ...
    await click(".app_priority a.app_priority_star.fa-star-o:last");
    await animationFrame();

    expect(".app_data_row:last-child .app_priority a.app_priority_star").toHaveCount(2, {
        message: "should still have two stars",
    });
    expect(".app_data_row:last-child .app_priority a.app_priority_star.fa-star").toHaveCount(2, {
        message: "should now have two full stars since the value is the third value",
    });
    expect(".app_data_row:last-child .app_priority a.app_priority_star.fa-star-o").toHaveCount(0, {
...
test("PriorityField hover in editable list view", async () => {
    ...
    // hover last star
    const star = ".app_data_row:first-child .app_priority a.app_priority_star.fa-star-o:last";
    await hover(star);
    await animationFrame();

    expect(".app_data_row:first-child .app_priority a.app_priority_star").toHaveCount(2);
    expect(".app_data_row:first-child .app_priority a.app_priority_star.fa-star").toHaveCount(2, {
        message: "should temporary have two full stars since we are hovering the third value",
    });
    expect(".app_data_row:first-child .app_priority a.app_priority_star.fa-star-o").toHaveCount(0, {
        message: "should temporary have no empty star since we are hovering the third value",
    });
...
test("PriorityField with readonly attribute", async () => {
    ...
    expect("span.app_priority_star.fa.fa-star-o.app_disabled").toHaveCount(2, {
        message: "stars of priority widget should rendered with span tag if readonly",
    });
    await hover(".app_priority_star.fa-star-o:last");
    await animationFrame();
    expect.step("hover");
    expect(".app_field_widget .app_priority a.app_priority_star.fa-star").toHaveCount(0, {
        message: "should have no full stars on hover since the field is readonly",
    });
    await click(".app_priority_star.fa-star-o:last");
    await animationFrame();
    expect.step("click");
    expect("span.app_priority_star.fa.fa-star-o.app_disabled").toHaveCount(2, {
        message: "should still have two stars",
...
test('PriorityField edited by the smart action "Set priority..."', async () => {
    ...
    await animationFrame();
    const idx = queryAllTexts(".app_command").indexOf("Set priority...\nALT + R");
    expect(idx).toBeGreaterThan(-1);
    await click(queryAll(".app_command")[idx]);
    await animationFrame();
    expect(queryAllTexts(".app_command")).toEqual(["Normal", "Blocked", "Done"]);
