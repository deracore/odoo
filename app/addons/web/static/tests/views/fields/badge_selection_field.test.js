...
test("BadgeSelectionField widget on a many2one in a new record", async () => {
    ...
    expect(`div.app_field_selection_badge`).toHaveCount(1, {
        message: "should have rendered outer div",
    });
    expect(`span.app_selection_badge`).toHaveCount(2, { message: "should have 2 possible choices" });
    expect(`span.app_selection_badge:contains(xphone)`).toHaveCount(1, {
        message: "one of them should be xphone",
    });
    expect(`span.active`).toHaveCount(0, { message: "none of the input should be checked" });

    await contains(`span.app_selection_badge`).click();
...
test("BadgeSelectionField widget on a selection in a new record", async () => {
    ...
    expect("span.app_selection_badge").toHaveCount(2, { message: "should have 2 possible choices" });
    expect(`span.app_selection_badge:contains(Red)`).toHaveCount(1, {
        message: "one of them should be Red",
    });

    await contains(`span.app_selection_badge:last`).click();
    await clickSave();
...
test("BadgeSelectionField widget on a selection in a readonly mode", async () => {
    ...
    expect(`div.app_readonly_modifier span`).toHaveCount(1, {
...
test("BadgeSelectionField widget on a selection unchecking selected value", async () => {
    ...
    expect("span.app_selection_badge").toHaveCount(2, { message: "should have 2 possible choices" });
    expect("span.app_selection_badge.active").toHaveCount(1, { message: "one is active" });
    expect("span.app_selection_badge.active").toHaveText("Red", {
        message: "the active one should be Red",
    });

    // click again on red option and save to update the server data
    await contains("span.app_selection_badge.active").click();
    expect.verifySteps([]);
...
test("BadgeSelectionField widget in list with the color_field option", async () => {
    ...
    // Ensure that the correct app_badge_color is used.
    expect(`.app_field_selection_badge[name="product_id"] .app_badge_color_6`).toHaveCount(1);
    expect(`.app_field_selection_badge[name="product_id"] .app_badge_color_7`).toHaveCount(0);
    expect(`div.app_field_selection_badge span:contains(xphone)`).toHaveCount(1);
    expect(`div.app_field_selection_badge span:contains(xpad)`).toHaveCount(0);

    // Open the M2O selection.
    await contains(`.app_field_selection_badge[name="product_id"] .app_badge_color_6`).click();
    ...
    expect(`.app_field_selection_badge[name="product_id"] .app_badge_color_6`).toHaveCount(0);
    expect(`.app_field_selection_badge[name="product_id"] .app_badge_color_7`).toHaveCount(1);
    expect(`div.app_field_selection_badge span:contains(xphone)`).toHaveCount(0);
    expect(`div.app_field_selection_badge span:contains(xpad)`).toHaveCount(1);
});
...
test("BadgeSelectionField widget in list without the color_field option", async () => {
    ...
    // Ensure that the 'btn btn-secondary' display is used instead of the 'app_badge_color' one.
    expect(`div.app_field_selection_badge span.btn-secondary`).toHaveCount(1);
    expect(`div.app_field_selection_badge span.btn-secondary:contains(xphone)`).toHaveCount(1);
    expect(`div.app_field_selection_badge span.btn-secondary:contains(xpad)`).toHaveCount(0);

    // Open the M2O selection.
    await contains(`div.app_field_selection_badge span:contains(xphone)`).click();
    ...
    expect(`div.app_field_selection_badge span.btn-secondary`).toHaveCount(1);
    expect(`div.app_field_selection_badge span.btn-secondary:contains(xphone)`).toHaveCount(0);
    expect(`div.app_field_selection_badge span.btn-secondary:contains(xpad)`).toHaveCount(1);
});
