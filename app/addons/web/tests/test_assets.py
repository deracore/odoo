...
from app.tests.common import HttpCase
from app.modules.module import get_manifest
...
class TestAssetsGenerateTimeCommon(app.tests.TransactionCase):
...
@app.tests.tagged('post_install', '-at_install', 'assets_bundle')
...
@app.tests.tagged('post_install', '-at_install', '-standard', 'test_assets')
class TestPregenerateTime(HttpCase):
    ...
    def test_logs_pregenerate_time(self):
        ...
        with self.profile(collectors=['sql', app.tools.profiler.PeriodicCollector(interval=0.01)], disable_gc=True):
            self.env['ir.qweb']._pregenerate_assets_bundles()
        duration = time.time() - start
        _logger.info('All bundle checked in %.2fs', duration)

@app.tests.tagged('post_install', '-at_install', '-standard', 'assets_bundle')
...
class TestLoad(HttpCase):
    def test_assets_already_exists(self):
        ...
        _save_attachment = app.addons.base.models.assetsbundle.AssetsBundle.save_attachment
        ...
        with patch('app.addons.base.models.assetsbundle.AssetsBundle.save_attachment', save_attachment):
