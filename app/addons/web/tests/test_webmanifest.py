...
class WebManifestRoutesTest(HttpCaseWithUserDemo):
    ...
    def test_webmanifest(self):
        ...
        self.assertCountEqual(data["icons"], [
            {'src': '/web/static/img/app-icon-192x192.png', 'sizes': '192x192', 'type': 'image/png'},
            {'src': '/web/static/img/app-icon-512x512.png', 'sizes': '512x512', 'type': 'image/png'}
        ])
    ...
    def test_webmanifest_scoped(self):
        ...
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': "/web/static/img/app-icon-192x192.png", 'sizes': 'any', 'type': 'image/png'}
        ])
        self.assertEqual(len(data["shortcuts"]), 0)
    ...
    def test_apple_touch_icon(self):
        ...
        response = self.url_open("/web/static/img/app-icon-ios.png")
        self.assertEqual(response.status_code, 200)

        document = self.url_open("/app")
        self.assertIn(
            '<link rel="apple-touch-icon" href="/web/static/img/app-icon-ios.png"/>', document.text,
