...
from app import api, fields, models, modules, tools, _
from app.exceptions import UserError, AccessError, RedirectWarning, ValidationError
from app.fields import Domain
from app.service import security
from app.http import request, root
from app.tools import config, is_html_empty, parse_version, split_every
from app.tools.barcode import check_barcode_encoding, createBarcodeDrawing, get_barcode_font
from app.tools.misc import find_in_path
from app.tools.pdf import PdfFileReader, PdfFileWriter, PdfReadError
from app.tools.safe_eval import safe_eval, time
...
def _wkhtml() -> WkhtmlInfo:
    ...
    try:
        ...
        if match:
            ...
            if config['workers'] == 1:
                _logger.info('You need to start DERACore with at least two workers to print a pdf version of the reports.')
                state = 'workers'
        else:
    ...
    try:
        ...
        if match:
            ...
            if config['workers'] == 1:
                _logger.info('You need to start DERACore with at least two workers to convert images to html.')
        else:
...
class IrActionsReport(models.Model):
    ...
    def _handle_merge_pdfs_error(self, error=None, error_stream=None):
        raise UserError(_("DERACore is unable to merge the generated PDFs."))
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if error_record_ids:
            ...
            raise RedirectWarning(
                message=_('DERACore is unable to merge the generated PDFs because of %(num_errors)s corrupted file(s)', num_errors=num_errors),
                action=action,
