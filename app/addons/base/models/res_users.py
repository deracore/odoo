...
from app import api, fields, models, tools, _
from app.api import SUPERUSER_ID
from app.exceptions import AccessDenied, AccessError, UserError, ValidationError
from app.fields import Command, Domain
from app.http import request, DEFAULT_LANG
from app.tools import email_domain_extract, is_html_empty, frozendict, reset_cached_properties, SQL
...
class ResUsers(models.Model):
    ...
    def _check_credentials(self, credential, env):
        ...
        * catch :class:`~app.exceptions.AccessDenied` and perform their
          own checking
        * (re)raise :class:`~app.exceptions.AccessDenied` if the
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by DERACore (updates, module installation, ...)'))
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: app.exceptions.AccessDenied when old password is wrong
        :raise: app.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this DERACore portal anymore.
