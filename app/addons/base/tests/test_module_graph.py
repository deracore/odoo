...
from app.modules.module_graph import ModuleGraph
from app.modules.module import _DEFAULT_MANIFEST, Manifest
from app.tools import mute_logger


class TestGraph(BaseCase):
    @mute_logger('app.modules.module_graph')
    def _test_graph_order(
        ...
        with (
            patch('app.modules.module_graph.ModuleGraph._update_from_database'),
            patch('app.modules.module_graph.Manifest.for_addon', make_manifest),
            patch('app.modules.module_graph.ModuleGraph._imported_modules', {'studio_customization'}),
        ):
            dummy_cr = None
