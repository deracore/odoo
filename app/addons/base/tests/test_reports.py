...
@app.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(app.tests.TransactionCase):
...
class TestReportsRenderingCommon(app.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        app.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@app.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@app.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
class TestAggregatePdfReports(app.tests.HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
