...
import app
from app.tests import TransactionCase
from app.tools import file_open, file_open_temporary_directory, file_path
from app.tools.config import configmanager

EMPTY_CONFIG_PATH = file_path('base/tests/config/empty.conf')
PROJECT_PATH = app.tools.config.root_path.removesuffix('/app')
DEFAULT_DATADIR = app.tools.config._default_options['data_dir']

MISSING_HTTP_INTERFACE = """\
WARNING:app.tools.config:missing --http-interface/http_interface, \
...
class TestConfigManager(TransactionCase):
    ...
    def test_01_default_config(self):
        self.assertConfigEqual({
            ...
            'proxy_access_token': '',
            'publisher_warranty_url': 'http://services.app.com/publisher-warranty/',
            'reportgz': False,
    ...
    def test_02_config_file(self):
        ...
        with self.assertLogs('app.tools.config', 'WARNING') as capture:
        ...
        self.assertEqual(capture.output, [
            "WARNING:app.tools.config:option addons_path, no such directory '/tmp/app', skipped",
            "WARNING:app.tools.config:option upgrade_path, no such directory '/tmp/upgrade', skipped",
            "WARNING:app.tools.config:option pre_upgrade_scripts, no such file '/tmp/pre-custom.py', skipped",
        ])
    ...
    def test_04_odoo16_config_file(self):
        # test that loading the DERACore 16.0 generated default config works
        ...
        with (
            ...
            self.assertLogs('app.tools.config', 'WARNING') as capture_warn,
        ):
        ...
        def missing(*options):
            return [
                f"WARNING:app.tools.config:unknown option '{option}' in "
        ...
        def falsy(*options):
            return [
                f"WARNING:app.tools.config:option {option} reads 'False' "
    ...
    def test_06_cli(self):
        ...
        self.assertEqual(capture.output, [
            "WARNING:app.tools.config:test file '/tmp/file-file' cannot be found",
        ])
        ...
        for args, stop_after_init in [
            ...
            with self.subTest(args=args):
                if any('--test' in arg for arg in args):
                    ...
                    self.assertEqual(capture.output, [
                        "WARNING:app.tools.config:Empty -d/--database/db_name, tests won't run",
                    ])
                else:
                    _, options = self.parse_reset(args)
