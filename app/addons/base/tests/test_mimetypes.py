...
from app.tests.common import BaseCase
from app.tools.mimetypes import fix_filename_extension, get_extension, guess_mimetype
...
class test_guess_mimetype(BaseCase):
    ...
    def test_default_mimetype_empty(self):
        ...
        # app implementation returns application/octet-stream by default
    ...
    def test_mimetype_svg(self):
        ...
        self.assertTrue(mimetype.startswith('image/svg'))
        # Tests that whitespace padded SVG are not detected as SVG in app implementation
        if not magic:
    ...
    def test_mimetype_fix_extension(self):
        ...
        with self.assertLogs('app.tools.mimetypes', 'WARNING') as capture:
            self.assertEqual(fix('image.txt', 'image/jpeg'), 'image.txt.jpg')
            self.assertEqual(fix('words.jpg', 'text/plain'), 'words.jpg.txt')
        self.assertEqual(capture.output, [
            "WARNING:app.tools.mimetypes:File 'image.txt' has an invalid "
                "extension for mimetype 'image/jpeg', adding '.jpg'",
            "WARNING:app.tools.mimetypes:File 'words.jpg' has an invalid "
