...
from app.cli.command import commands, load_addons_commands, load_internal_commands
from app.tests import BaseCase
from app.tools import config, file_path
...
class TestCommand(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.odoo_bin = Path(__file__).parents[4].resolve() / 'app-bin'
    ...
    def test_docstring(self):
        ...
        for name, cmd in commands.items():
            self.assertTrue(cmd.__doc__,
                msg=f"Command {name} needs a docstring to be displayed with 'app-bin help'")
            self.assertFalse('\n' in cmd.__doc__ or len(cmd.__doc__) > 120,
                msg=f"Command {name}'s docstring format is invalid for 'app-bin help'")
        ...
        for name in ('bonbon', 'café'):
            with self.subTest(name):
                ...
                self.assertEqual(
                    ...
                    "Use 'app-bin --help' to see the list of available commands."
                )
    ...
    def test_upgrade_code_standalone(self):
        from app.cli import upgrade_code  # noqa: PLC0415
        proc = sp.run(
