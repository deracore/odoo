...
from app.tests.common import get_db_name, tagged
...
def get_odoo_module_name(python_module_name):
    if python_module_name.startswith('app.addons.'):
        return python_module_name.split('.')[2]
    if python_module_name == 'app.models':
        return 'app'
...
class TestLintOverrideSignatures(LintCase):
    def test_lint_override_signature(self):
        ...
        #with open('/tmp/app-override-signatures.md', 'w') as f:
