...
from app.fields import Command, Date, Datetime
from app.http import Controller, Response, dispatch_rpc, request, route
from app.tools import lazy
from app.tools.misc import frozendict
...
# User code must use the exceptions defined in ``app.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, app.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, app.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, app.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, app.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, app.exceptions.MissingError):
    ...
    # InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(app.tools.exception_to_unicode(e), formatted_info)
...
class OdooMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, app is sending binary as base64 string.
