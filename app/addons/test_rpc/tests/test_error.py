...
from app.tools.misc import mute_logger
...
class TestError(common.HttpCase):
    def setUp(self):
        ...
        ml = mute_logger('app.addons.rpc.controllers.xmlrpc')
    ...
    def test_01_private(self):
        with self.assertRaisesRegex(Exception, r"Private method"), mute_logger('app.http'):
    ...
    def test_01_create(self):
        ...
        with self.assertRaises(Fault) as ctx, mute_logger("app.sql_db", "app.http"):
    ...
    def test_03_sql_constraint(self):
        with mute_logger("app.sql_db"), self.assertLogs("app.http", level="WARNING") as capture:
