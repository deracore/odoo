...
QUnit.test("can be rendered", async (assert) => {
    ...
    assert.containsOnce(
        target,
        ".app_grid_apps_menu button.app_grid_apps_menu__button",
...
QUnit.test("can be opened and closed", async (assert) => {
    const env = await makeTestEnv(baseConfig);
    await mount(NavBar, target, {env});
    await click(target, "button.app_grid_apps_menu__button");
    await nextTick();
    assert.containsOnce(target, ".app-menu-list");
    await click(target, "button.app_grid_apps_menu__button");
    await nextTick();
    assert.containsNone(target, ".app-menu-list");
});
...
QUnit.test("can be active", async (assert) => {
    ...
    await nextTick();
    assert.containsOnce(target, '.app-menu-item.active[data-menu-xmlid="menu_1"]');
    env.services.menu.setCurrentMenu(2);
    await nextTick();
    assert.containsOnce(target, '.app-menu-item.active[data-menu-xmlid="menu_2"]');
});
