...
import {Component, onPatched, onWillPatch, useRef, useState} from "@app/owl";
import {
        ...
        for (const root in this.rootMenuItems) {
            ...
            if (this.rootMenuItems[root]?.actionPath) {
                this.rootMenuItems[root].path =
                    `/app/${this.rootMenuItems[root].actionPath}`;
            }
            // Root is a module
            else {
                this.rootMenuItems[root].path =
                    `/app/action-${this.rootMenuItems[root].actionID}`;
            }
        }
                ...
                if (this.subMenuItems[item].appID === this.rootMenuItems[root].appID) {
                    ...
                    if (this.rootMenuItems[root]?.actionPath) {
                        this.subMenuItems[item].path =
                            `/app/${this.rootMenuItems[root].actionPath}/action-${this.subMenuItems[item].actionID}`;
                    }
                    // Root is a module
                    else {
                        this.subMenuItems[item].path =
                            `/app/action-${this.subMenuItems[item].actionID}`;
                    }
                }
            }
        }
