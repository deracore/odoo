...
from app.sql_db import close_db, db_connect
from app.tests import HOST, BaseCase, Like, get_db_name, tagged
from app.tools import mute_logger, reset_cached_properties, SQL
from app.tools.urls import urljoin
...
class TestHttpRegistry(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.addClassCleanup(reset_cached_properties, app.http.root)
        cls.classPatch(app.conf, 'server_wide_modules', ['base', 'web', 'rpc', 'test_http'])

        # make sure there are always many databases, to break monodb
        cls._db_list = cls.startClassPatcher(patch('app.http.db_list'))
        cls._db_list.return_value = ['postgres', get_db_name()]
        cls.startClassPatcher(patch('app.http.db_filter',
    ...
    def authenticate(self, *, db=None):
        session = app.http.root.session_store.new()
        session.update(app.http.get_default_session(), db=db or get_db_name())
    ...
    def url_open(self, path, *, allow_redirects=False):
        ...
        url = urljoin(f"http://{HOST}:{app.tools.config['http_port']}", path)
    ...
    def test_signaling(self):
        ...
        with self.assertLogs('app.registry', logging.INFO) as capture:
            res = self.url_open('/test_http/ensure_db')
            self.assertEqual(res.status_code, 200)
        self.assertEqual(capture.output, [
            "INFO:app.registry:Reloading the model registry after database signaling.",
            Like("INFO:app.registry:Registry loaded in ...s"),
        ])
    ...
    def test_missing_db(self):
        ...
        with self.assertLogs('app.http', logging.WARNING) as capture:
            res = self.url_open('/test_http/ensure_db')
        ...
        self.assertEqual(capture.output, [
            Like("WARNING:app.http:Database or registry unusable, trying without\n"
                 f'Traceback...database "{db_duplicate}" does not exist...')
        ] * 2)

    @mute_logger('app.sql_db')
    def test_corrupt_ir_module_module_table(self):
        ...
        with self.assertLogs('app.registry', logging.ERROR) as capture1, \
             self.assertLogs('app.http', logging.WARNING) as capture2:
            res = self.url_open('/test_http/greeting-public')
            self.assertEqual(res.status_code, 404)
        self.assertEqual(capture1.output, [
            "ERROR:app.registry:Failed to load registry",
        ])
