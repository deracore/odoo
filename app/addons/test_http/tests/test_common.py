...
from app.tools import config, reset_cached_properties
from app.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(app.http.Application, 'session_store', session_store)
        cls.classPatch(app.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(app.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        app.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('app.http.db_list') as db_list, \
             patch('app.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('app.http.db_list') as db_list, \
             patch('app.http.db_filter') as db_filter, \
             patch('app.http.Registry') as Registry:
