...
from app.tests import get_db_name, tagged
from app.tools import config, mute_logger, reset_cached_properties

from .test_common import TestHttpBase
from app.addons.base.tests.common import HttpCase, HttpCaseWithUserDemo
from app.addons.test_http.controllers import CT_JSON
...
class TestHttpSession(TestHttpBase):
    ...
    @mute_logger('app.http')  # greeting_none called ignoring args {'debug'}
    ...
    def test_session01_default_session(self):
        ...
        with patch.object(app.http.root.session_store, 'save') as mock_save:
    ...
    def test_session03_logout_15_0_geoip(self):
        ...
        with self.assertLogs('app.http', level='WARNING') as (_, warnings):
            res = self.multidb_url_open('/test_http/ensure_db', dblist=['db1', 'db2'])

        self.assertEqual(warnings, [
            "WARNING:app.http:Logged into database 'idontexist', but dbfilter rejects it; logging session out.",
        ])
    ...
    @patch("app.http.root.session_store.vacuum")
    ...
    def test_session09_logout(self):
        ...
        self.assertTrue(app.http.root.session_store.get(sid), "session should exist")
        self.url_open('/web/session/logout', allow_redirects=False).raise_for_status()
        self.assertFalse(app.http.root.session_store.get(sid), "session should not exist")
    ...
    def test_session10_explicit_session(self):
        ...
        with self.assertLogs('app.http') as capture:
        ...
        self.assertRegex(capture.output[0],
            r"^WARNING:app.http:<function app\.addons\.\w+\.controllers\.\w+\.logout> "
    ...
    def test_session15_x_odoo_database_with_prior_session_different(self):
        ...
        self.assertIn(
            "Cannot use both the session_id cookie and the x-app-database header.",
            res.text,
        )
    ...
    def test_session16_soft_rotate_with_abort(self):
        ...
        # the abort in app.http.JsonRPCDispatcher.dispatch.
...
class TestSessionStore(HttpCaseWithUserDemo):
    def setUp(self):
        ...
        reset_cached_properties(app.http.root)
        self.addCleanup(reset_cached_properties, app.http.root)
        patcher = patch.dict(config.options, {'data_dir': self.tmpdir.name})
        self.startPatcher(patcher)

    @mute_logger('app.http')
    def test01_session_nan(self):
        ...
        with self.assertLogs('app.http', level='WARNING') as logs:
            self.assertEqual(app.http.get_session_max_inactivity(self.env), SESSION_LIFETIME)
            self.assertEqual(logs.output[0], "WARNING:app.http:Invalid value for 'sessions.max_inactivity_seconds', using default value.")
    ...
    def test02_session_lifetime_1week(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = app.http.root.session_store.get(session.sid)
    ...
    def test04_session_lifetime_nodb(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = app.http.root.session_store.get(session)
