...
import app
from app.http import content_disposition, root
from app.tests import tagged
from app.tests.common import HOST, BaseCase, get_db_name, new_test_user
from app.tools import config, file_path, mute_logger, parse_version

from .test_common import TestHttpBase
from app.addons import test_http
from app.addons.test_http.controllers import CT_JSON
from app.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = app.tools.misc.DotDict(db=False)
            return app.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc4_rpc_qweb(self):
        ...
        for method in (self.db_url_open, self.nodb_url_open):
            with self.subTest(method=method.__name__):
                with mute_logger('app.addons.rpc.controllers.jsonrpc'):
    ...
    def test_misc8_concurrency_error(self):
        with (
            self.assertLogs('app.service.model') as log_catcher,
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(app.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = app.http.root.session_store.get(res.cookies['session_id'])
    ...
    def test_ensure_db4_unicode(self):
        ...
        self.assertEqual(
            app.http.root.session_store.get(res.cookies['session_id']).db,
            'basededonnée1')
