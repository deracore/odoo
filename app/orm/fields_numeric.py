...
from app.tools import float_compare, float_is_zero, float_repr, float_round
from app.tools.misc import SENTINEL, Sentinel
...
class Float(Field[float]):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~app.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~app.fields.Float.round()` to round a float with the given precision.
    :func:`~app.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~app.fields.Float.compare()` to compare two floats at the given precision.
...
class Monetary(Field[float]):
    ...
    :class:`res_currency<app.addons.base.models.res_currency.Currency>`.
    ...
    :param str currency_field: name of the :class:`Many2one` field
        holding the :class:`res_currency <app.addons.base.models.res_currency.Currency>`
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            # FIXME @rco-app: currency may not be already initialized if it is
    ...
    def _filter_not_equal(self, records: BaseModel, cache_value: typing.Any) -> BaseModel:
        ...
        # see fix app/app#177200 (commit 7164d5295904b08ec3a0dc1fb54b217671ff531c)
        env = records.env
