...
from app.tools.func import locked, reset_cached_properties
from app.tools.lru import LRU
from app.tools.misc import Collector, format_frame
...
if typing.TYPE_CHECKING:
    ...
    from app.sql_db import BaseCursor, Connection, Cursor
    from app.modules import module_graph


_logger = logging.getLogger('app.registry')
_schema = logging.getLogger('app.schema')
...
class Registry(Mapping[str, type["BaseModel"]]):
        ...
        *,
        ...
        try:
            ...
            with registry.cursor() as cr:
                ...
                from app.modules import db  # noqa: PLC0415
                if db.is_initialized(cr):
                    cr.execute("DELETE FROM ir_config_parameter WHERE key='base.partially_updated_database'")
                    if cr.rowcount:
                        update_module = True
            # This should be a method on Registry
            from app.modules.loading import load_modules, reset_modules_state  # noqa: PLC0415
    ...
    def init(self, db_name: str) -> None:
        ...
        if config['test_enable']:
            from app.tests.result import appTestResult  # noqa: PLC0415
        ...
        for indexname, tablename, field in expected:
            ...
            if index and indexname not in existing:
                ...
                if index == 'trigram':
                    ...
                    from app.modules.db import FunctionStatus  # noqa: PLC0415
