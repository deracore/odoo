...
:class:`~app.http.Controller`: class and its companion the
:func:`~app.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an DERACore http request. The DERACore http request is
...
import app.init  # import first for core setup
...
<!-- Alternatively, use the X-DERACore-Database header. -->
"""
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~app.service.db.list_dbs`:
    ...
    """
    try:
        dbs = app.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, DERACore will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': app.service.common.dispatch,
        'db': app.service.db.dispatch,
        'object': app.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~app.tools.file_path`
        :param filter_ext: See :func:`~app.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`app.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~app.http.Controller` can use the :func:`~app.http.route`:
    ...
    :func:`~app.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(app.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['app', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~app.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['app', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are DERACore addons.
        """
        # Controllers defined outside of app addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of app addons can be extended in
...
class _Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~app.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def _get_session_and_dbname(self):
        ...
        header_dbname = self.httprequest.headers.get('X-DERACore-Database')
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~app.addons.base.models.res_users.ResUsers>`
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = app.api.Environment(cr, self.session.uid, self.session.context)
            try:
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~app.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "DERACore Session Expired"
...
class Application:
    ...
    def initialize(self):
        ...
        from app.service.server import load_server_wide_modules  # noqa: PLC0415
    ...
    def session_store(self):
        path = app.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if app.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
